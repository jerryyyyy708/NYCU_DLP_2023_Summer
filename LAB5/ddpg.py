    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                if ewma_reward > best:
                    best = ewma_reward
                    agent.save(f'models/ddpg_best.pth')
                break
        if total_reward > args.thresh:
            agent.save(f'models/ddpg{episode}.pth')
...
def main():
    ...
    if not args.test_only:
        try:
            ...
            agent.save(f'ddpg_Interrupted_{args.model}')
            args.model = f'ddpg_Interrupted_{args.model}'
